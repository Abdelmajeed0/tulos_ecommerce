...
function SearchBar() {
  ...
  return (
    <Dialog open={showSearch} onOpenChange={() => setShowSearch(!showSearch)}>
      ...
      <DialogContent className="md:max-w-5xl min-h-[90vh] max-h-[90vh] flex flex-col overflow-hidden bg-white">
        <DialogHeader>
          ...
          <form className="relative" onSubmit={(e) => e.preventDefault()}>
            ...
            <button
              type="submit"
              className={`absolute right-0 top-0 w-10 h-full flex items-center justify-center rounded-tr-md hover:bg-gray-900 hover:text-white hovereffect ${search ? "bg-gray-900 text-white" : "bg-gray-900/10 "}`}
            >
              <Search className="w-5 h-5" />
            </button>
          </form>
        </DialogHeader>
        <div className="w-full h-full overflow-y-scroll border border-gray-900/20  rounded-md">
          <div>
            {loading ? (
              ...
              products?.map((product: Product) => (
                ...
                >
                  <div className="flex items-center p-1">
                    <Link
                      className="h-20 w-20 md:h-24 md:w-24 flex-shrink-0 border border-gray-900/20 rounded-md overflow-hidden group"
              ...
              <div className="text-center py-10 font-semibold tracking-wide">
                {search && !loading ? (
                  ...
                  <p className="text-green-600 flex items-center justify-center gap-1">
                    <Search className="w-5 h-5" /> Search and explore product
                    from Tulos{" "}
                  </p>
                )}
              </div>
